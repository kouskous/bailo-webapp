...
export class DocumentsList {
  ...
  get filteredDocuments(): LeaseDocument[] {
    const scheduleDocuments = this.documents.filter(
      (document) =>
    ...
    return scheduleDocuments.filter(
      (document) => document.type === this.selectedType,
    );
  }
  ...
  openDocument(document: LeaseDocument): void {
    ...
    this.documentService
      .getDocumentViewBlob(document.id)
      .pipe(take(1))
  ...
  downloadDocument(document: LeaseDocument): void {
    ...
    this.documentService
      .getDocumentDownloadBlob(document.id)
      .pipe(take(1))
      .subscribe((blob) =>
